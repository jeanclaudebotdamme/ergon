...
export default function KanbanColumn({ column, tasks }: KanbanColumnProps) {
  ...
  return (
    <div className="w-column flex-shrink-0 bg-surface rounded-xl p-4 flex flex-col">
      ...
      {/* Add Card Button */}
      <div className="mt-3 p-2.5 border border-dashed border-border-hover rounded-lg text-[13px] text-[#555] cursor-pointer text-center transition-all duration-150 hover:border-[#4a4a4a] hover:text-[#777] hover:bg-white/[0.02]">
        + Add task
      </div>
    </div>
  );
